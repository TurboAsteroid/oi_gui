...
const baseAddress = 8585; // этот адрес необходимо сказать пользователю
const redirectAddress = 8523; // служебный адрес http. по прямому обращению не работает
const httpsAddress = 8524; // служебный адрес https. по прямому обращению работает
...
server.listen(httpsAddress, () => {
  console.log('Vue.js static https server started at port', httpsAddress);
});

// пользовательский проксирующий сервер
net.createServer(tcpConnection).listen(baseAddress, () => {
  console.log('Vue.js static base server started at port', baseAddress);
});
